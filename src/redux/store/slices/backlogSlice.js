...
export const getBacklogAction = createAsyncThunk(
  "backlog/getAll",
  async (projectId) => {
...
export const updateTaskStatusAction = createAsyncThunk(
  ...
  async ({ projectId, taskId, newStatus }) => {
    ...
    axios.put("http://localhost:8000/projects/" + projectId, {
      ...res.data,
      tasks: array,
    });
    return array;
...
export const DelteTaskAction = createAsyncThunk(
  "backlog/DeleteTask",
  async ({ projectId, deletedTaskId }) => {
    const res = await axios.get("http://localhost:8000/projects/" + projectId);
...
export const UpdateTaskDateAction = createAsyncThunk(
  "backlog/UpdateTaskDate",
  async ({ projectId, taskId, startDate, endDate }) => {
    const res = await axios.get("http://localhost:8000/projects/" + projectId);
    const array = res.data.tasks.map((t) => {
      if (t.id === taskId) {
